...
def eq_hamilton_J(beta,time,epsilon=0.0,factor=1.0,gamma=1.0):
    def bimodal_mu_lam(q,t,lam,eps,factor,g):
        epsilon_lam, epsilon_mu = eps[0], eps[1]
        dy1_dt_sus_inf = lam * ((1 - epsilon_lam) * q[0] + (1 + epsilon_lam) * q[1]) * (1 - epsilon_mu) * (
                1 / 2 - q[0]) * np.exp(q[2]) - g * q[0] * np.exp(-q[2])
        dy2_dt_sus_inf =  lam * ((1 - epsilon_lam) * q[0] + (1 + epsilon_lam) * q[1]) * (1 + epsilon_mu) * (
                1 / 2 - q[1]) * np.exp(q[3]) - g * q[1] * np.exp(-q[3])
        dtheta1_dt_sus_inf = -lam * (1 - epsilon_lam) * (
        ...
        dtheta2_dt_sus_inf =  -lam * (1 + epsilon_lam) * (
                (1 - epsilon_mu) * (1 / 2 - q[0]) * (np.exp(q[2]) - 1) + (1 + epsilon_mu) * (1 / 2 - q[1]) * (
                np.exp(q[3]) - 1)) + lam * ((1 - epsilon_lam) * q[0] + (1 + epsilon_lam) * q[1]) * (
                                    1 + epsilon_mu) * (np.exp(q[3]) - 1) - g * (np.exp(-q[3]) - 1)
        return np.array([dy1_dt_sus_inf, dy2_dt_sus_inf, dtheta1_dt_sus_inf, dtheta2_dt_sus_inf])
...
def postive_eigen_vec(J,q0,lam,eps,factor,gamma):
    # Find eigen vectors
    eigen_value, eigen_vec = la.eig(J(q0,0.0,lam,eps,factor,gamma))
    postive_eig_vec = []
...
def shoot(y1_0, y2_0, p1_0, p2_0, tshot, J,dq_dt,lam,epsilon,factor,gamma):
    ...
    qsol = odeint(dq_dt, q0, tshot,args=(lam,epsilon,factor,gamma),Dfun=vect_J)
    return qsol


def one_shot(shot_angle,lin_weight,q_star,radius,final_time_path,one_shot_dt,J,shot_dq_dt,epsilon,lam,factor,gamma):
    q0 = (q_star[0] + radius * np.cos(shot_angle), q_star[1], 0+radius * np.sin(shot_angle), 0)
    postive_eig_vec = postive_eigen_vec(J, q0,lam,epsilon,factor,gamma)
...
if __name__=='__main__':
    ...
    path = one_shot(angle, int_lin_combo, q_star, r, t, dt, J, shot_dq_dt,epsilon,lam,factor,gamma)
